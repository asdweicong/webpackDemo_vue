    ...
    <section class="logo">
        ...
        <section class="userLogin">
                <section class="userUsBtn">
                	<router-link to="/user/regist" class="userrelogin">没有账号? 点我注册</router-link>
                     <!--<a href="#/user/regist" class="userrelogin">没有账号? 点我注册</a>-->
                     <Button type="primary" v-on:click="loginbtn()">登录</Button>
                </section>
        </section>
    </section>
</template>
            ...
            methods:{
                loginbtn:function(){
                    if(this.userinputName == null ||this.userinputPassword == null){
						...
						this.$axios.post('http://127.0.0.1:3000/user/login',data)
						.then(response => {
							console.log(response)
						    if(response.data.status === 0){
						    	this.instance('error','该用户名未注册！')
						    }else if(response.data.status === 1){
						    	this.instance('error','密码错误！')
						    }else if(response.data.status === 2){
						    	this.instance('success','登录成功')
						    }
						})
